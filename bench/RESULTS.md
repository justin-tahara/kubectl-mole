...
- Wall clock is proof versus photo: mole's tool overhead is under 100ms (see the per-scenario phase lines) — the rest of its wall is the evidence window. The snapshot baselines are timed after the failure is already steady, so the wait that got the operator there is not in their column.
